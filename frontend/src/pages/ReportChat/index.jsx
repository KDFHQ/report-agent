...
export default observer(function ReportChat() {
  ...
  const relatedIndex = useRef(null)
  const { type } = useParams();
  const type_obj = {
    cn: {
  ...
  async function sendMessage(question) {
    ...
    const new_session = {
      ...
      messages: [...chat_store.selected_session.messages, {
        role: "user",
        content: question,
        index_name: type_obj[type].index_name,
        timestamp: createFormattedTime()
  ...
  const relatedOnClick = (
    ...
    tabActive.current = "yinyong";
    relatedIndex.current = index;
    imageData.current = {
      query: msg,
      paraId: paraId,
      ai_type: index_name,
  ...
  return (
    <div className="flex w-full h-full">
      ...
      />
      <Chat className="flex-1" data={chat_store.selected_session ? chat_store.selected_session.messages : []} onSendMessage={sendMessage} />
    </div>
  );
})
